...
export function useChat(language: Language) {
  ...
  const send = useCallback(
    async (text: string) => {
      ...
      try {
        ...
        setError(language === 'sw' ? 'Kuna hitilafu. Tafadhali jaribu tena.' : 'Something went wrong. Please try again.');
      } finally {
        setIsLoading(false);
      }
    },
  ...
  return { messages, isLoading, error, detectedArea, suggestLetter, send, clear };
}
